...
function removeArtist(array, index){
  return array.splice(index, 1), array.length;
}
...
function addArtist(array){
  array.push({
    ...
    bio: 'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Doloremque laborum velit incidunt aliquam ratione, totam, itaque voluptates commodi quo quas laudantium, maxime dolorem iusto dignissimos. Fugit corporis amet consequatur dolore.'
  });
  return array;
}
...
function getHTML(data){
  for (const i in data) {
    if (data[i].wikipedia) {
      ...
      <div id='${data[i].id}'>
        <div class='image'>
          <img src='assets/${data[i].name.split(' ')[0]}-${data[i].name.split(' ')[1]}.jpg'>
        </div>
        <div class = 'name>
...
const filter = artists.filter(data => data.years.toString().startsWith('19'));
const map = filter.map(artist => artist.name);
